...
For the Pollo / Seedance sponsor workflow, start with the Reels tab and click `Big 10` to show the prepared references inside the app. The broader search categories are `UGC`, `Product Promo`, `Ads`, `Reviews`, `Unboxing`, `Explainers`, and `AI Creator`. See [`VIDEO_IDEAS.md`](VIDEO_IDEAS.md) for tested reference links and prompt starters.
